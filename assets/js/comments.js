...
/*
  ...
  \_| |_|___/\__|_|  \__,_|
    ...
    Astra was here! Modified this a bit to fit my site. 
    Go check out the linked guide above for the original!
*/

// The values in this section are REQUIRED for the widget to work! Keep them in quotes!

// const isOnCommentsPage = (new URL(window.location.href).pathname === "/comments/");

const s_stylePath = 'PATH HERE';
const s_formId = '1FAIpQLSdNzvxsuP66kwFsA1LOW3lqvkJHDYfjGTSSr37WvXrP28CwxA';
const s_nameId = '2053066708';
const s_websiteId = '2095649877';
const s_textId = '129527008';
const s_pageId = '688929575';
const s_replyId = '2070610580';
const s_sheetId = '1BZQCkqv7Y_P5i47wVbjwhje87qg_SZcRPF_nhRfySDo';
...
const s_filteredWords_DEF = [ // Add words to filter by putting them in quotes and separating with commas (ie. 'heck', 'dang')
    // 'heck', 'dang'
    '____________preventEmpty',
]

const s_encodedFilteredWords = [
    // btoa + rot13

];

const s_filteredWords = s_encodedFilteredWords.length === 0 ? s_filteredWords_DEF : s_encodedFilteredWords.map(i => atob(i.replace(/[a-zA-Z]/g, function (c) { return String.fromCharCode((c <= "Z" ? 90 : 122) >= (c = c.charCodeAt(0) + 13) ? c : c - 26); })))
...
function handleSubmit(form) {
    form.submit();
    document.getElementById('c_submitButton').style = "display: none;"
...
// date stuff
// Function to format and set the current date and time
    ...
    retrievedSheet.then(result => {
        ...
        if (json.table.parsedNumHeaders > 0) { // Check if any comments exist in the sheet at all before continuing
            for (r = 0; r < json.table.rows.length; r++) {
                ...
                // if (val1 == v_pagePath || true) {  // || true disables this and just shows all comments
                if (val1 == v_pagePath) {  // || true disables this and just shows all comments
                    let comment = {}
...
function createComment(data) {
    ...
    authorInfo.style = "display: flex; flex-direction: column;";
    ...
    //     site.innerText = ` (from ${data.Website.split("://").map((i, idx) => idx === 0 ? i.replace(/http/g, 'hxxp') : i).join("://").replace(/\./g, "[.]")})`; //s_websiteText;
    //     // site.href = data.Website.split("://").map((i, idx) => idx === 0 ? i.replace(/http/g, 'hxxp') : i).join("://").replace(/\./g, "[.]");
...
// if(isOnCommentsPage){
//     getComments(); // Run once on page load, but only on the comments page
// }
getComments();